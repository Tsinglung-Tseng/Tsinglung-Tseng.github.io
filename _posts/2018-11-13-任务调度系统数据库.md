...
```SQl
-- CREATE TYPE depends_result AS (tasks_id int, dependency integer[]);

-- DROP FUNCTION get_depends(taskID INTEGER);
--
-- create or replace function get_depends(taskID INT)
-- returns table(simu_id int, task_id int, depends integer[]) as $$
--   SELECT "taskSimu".id, tasks.id, tasks.depends
--   FROM "taskSlurm"
